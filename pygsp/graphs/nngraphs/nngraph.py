...
def _scipy_ckdtree(features, _, order, kind, k, radius, params):
    ...
    params = dict(p=order, eps=eps, workers=-1)
    if kind == "knn":
        params["k"] = k + 1
    elif kind == "radius":
...
def _flann(features, metric, order, kind, k, radius, params):
    ...
    try:
        ...
        raise ImportError(
            "Cannot import cyflann. Choose another nearest "
...
def _nmslib(features, metric, order, kind, k, _, params):
    ...
    try:
        ...
        raise ImportError(
            "Cannot import nmslib. Choose another nearest "
            "neighbors backend or try to install it with "
        ...
        **kwargs
        ...
        if kind == "knn":
            if not 1 <= k < n_vertices:
                raise ValueError(
                    "The number of neighbors (k={}) must be "
        ...
        neighbors, distances = function(
            features, metric, order, kind, k, radius, kwargs
        )
        ...
        if kind == "radius":
            ...
            if n_disconnected > 0:
                _logger.warning(
                    "{} vertices (out of {}) are disconnected. "
